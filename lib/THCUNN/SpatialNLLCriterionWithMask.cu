...
__global__ void cunn_SpatialNLLCriterionWithMask_updateOutput_kernel(
          T *output,
          T *total_weight,
          T *input,
          THCIndex_t *target,
          ...
          int blocks_per_sample)
{
  ...
  for (i = (blockIdx.x % blocks_per_sample) * blockDim.x + threadIdx.x;
       i < map_nelem;
       i += step) {
    if(valid_mask[toffset + i] < ScalarConvert<float, T>::to(1.0))
      continue;
...
template<typename T>
__global__ void cunn_SpatialNLLCriterionWithMask_updateGradInput_kernel(
          T *gradInput,
          THCIndex_t *target,
          T *valid_mask,
          T *weights,
...
#include "generic/SpatialNLLCriterionWithMask.cu"
